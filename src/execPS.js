...
export function execPS (fileName, payload = {}) {
	const scriptPath = path.join(__dirname, "scripts", fileName)
	const fullPath = path.resolve(scriptPath)
	
	const jsonArg = JSON.stringify(payload)
	const cmd = `powershell.exe -NoProfile -ExecutionPolicy Bypass -File "${fullPath}" -json '${jsonArg}'`
	...
	return new Promise((resolve, reject) => {
		exec(cmd, { maxBuffer: 1024 * 500 }, (err, stdout, stderr) => {
			...
			try {
				...
				console.error("[execPS] JSON parsing error:", err.message)
